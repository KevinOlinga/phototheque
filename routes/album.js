...
router.get('/:albumId/imgs/new', async (req, res) => {
  try {
    const albumId = req.params.albumId;
    const album = await Album.findById(albumId);
    res.render('new_img', { albumId, albumTitle: album.title, cssFile: 'new_img.css' });
  } catch (error) {
    console.error(error);
...
router.post('/albums/imgs/:albumId', upload.single('image'), async (req, res) => {
  try {
    const albumId = req.params.albumId;
    ...
    res.redirect(`/albums/${albumId}/imgs`);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Une erreur est survenue ...' });
  }
